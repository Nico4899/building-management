 ...
 * by lexicographical number order.
 */
public class LexicographicNumberRoomSorter implements Sorter<Room> {
  @Override
  public Collection<Room> sort(Collection<Room> collection) {
    return collection.stream().sorted(Comparator.comparing(Room::getRoomNumber)).toList();
  }
}
