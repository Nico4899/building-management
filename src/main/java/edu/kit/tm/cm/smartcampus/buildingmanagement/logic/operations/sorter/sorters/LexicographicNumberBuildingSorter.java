 ...
 * Building} by lexicographical number order.
 */
public class LexicographicNumberBuildingSorter implements Sorter<Building> {
  @Override
  public Collection<Building> sort(Collection<Building> collection) {
    return collection.stream().sorted(Comparator.comparing(Building::getBuildingNumber)).toList();
  }
}
